...
class PODAnalyzer(BaseAnalyzer):
    """Proper Orthogonal Decomposition analyzer."""
    ...
    def __init__(self,
        ...
        Args:
            ...
            spatial_weight_type (str): Type of spatial weights ('auto', 'uniform', 'polar').
            n_modes_save (int): Number of POD modes to save and consider for plotting.
        """
        ...
        self.time_coeffs = np.array([]) # Temporal coefficients (Psi)
    ...
    def perform_pod(self):
        """Perform POD analysis on the loaded data."""
        ...
        if Ns < Nspace:
            ...
            self.time_coeffs = temporal_coeffs_unscaled * np.sqrt(self.eigenvalues * Ns) # Scale temporal coefficients
        ...
        self.time_coeffs = self.time_coeffs[:, :self.n_modes_save]
    ...
    def save_results(self, filename=None):
        """Save POD modes and eigenvalues to HDF5 file."""
        if not filename:
        ...
        with h5py.File(save_path, "w") as f:
            ...
            if 'x' in self.data: f.create_dataset("x", data=self.data['x'], compression="gzip")
            if 'y' in self.data: f.create_dataset("y", data=self.data['y'], compression="gzip")
            if self.W.size > 0: f.create_dataset("W", data=self.W, compression="gzip")
            if self.temporal_mean.size > 0: f.create_dataset("temporal_mean", data=self.temporal_mean, compression="gzip")
            ...
            f.create_dataset("time_coeffs", data=self.time_coeffs, compression="gzip") # Psi (temporal coefficients)
    ...
    def plot_modes(self, n_modes_to_plot=4):
        ...
        if Nx * Ny != self.modes.shape[0]:
             print(f"Warning: Nx ({Nx}) * Ny ({Ny}) != Nspace ({self.modes.shape[0]}). Cannot reshape modes for 2D plot. Plotting as 1D.")
             # Fallback or error, for now just print warning and continue if possible for 1D

        # Assuming X and Y are meshgrid-compatible if 2D plot is intended
    ...
    def plot_time_coefficients(self, n_coeffs_to_plot=2, n_snapshots_plot=None):
        ...
        if self.time_coeffs.size == 0:
            print("No time coefficients to plot. Run perform_pod() first.")
            return

        n_coeffs_to_plot = min(n_coeffs_to_plot, self.time_coeffs.shape[1], self.n_modes_save)
        if n_coeffs_to_plot == 0:
            print("No coefficients available to plot.")
            return

        Ns_total = self.time_coeffs.shape[0]
        ...
        for i in range(n_coeffs_to_plot):
            ...
            plt.plot(time_vector, self.time_coeffs[:n_snapshots_plot, i], linewidth=1.5)
            plt.xlabel('Time')
    ...
    def plot_reconstruction_error(self):
        ...
        if self.modes.size == 0 or self.time_coeffs.size == 0 or 'q' not in self.data:
        ...
        for k in range(1, n_modes_check + 1):
            ...
            # self.time_coeffs is (Ns, n_modes_save)
            # self.modes is (Nspace, n_modes_save)
            reconstructed_data_k_modes = self.time_coeffs[:, :k] @ self.modes[:, :k].T
    ...
    def plot_reconstruction_comparison(self, snapshot_indices_to_plot=None, modes_for_reconstruction=None):
        ...
        Args:
            snapshot_indices_to_plot (list of int, optional): Indices of snapshots to plot.
                                                              Defaults to [0, Ns//2, Ns-1].
            modes_for_reconstruction (list of int, optional): Number of modes to use for reconstructions.
                                                             Defaults to [1, n_modes_save//2, n_modes_save].
        """
        if self.modes.size == 0 or self.time_coeffs.size == 0 or 'q' not in self.data:
            print("Data, modes, or time coefficients not available. Run perform_pod() first.")
            return
        ...
        fig, axes = plt.subplots(num_snapshots_to_show, 1 + num_recons_per_snapshot, 
        ...
        for i, snap_idx in enumerate(snapshot_indices_to_plot):
            ...
            for j, k_modes in enumerate(modes_for_reconstruction):
                ...
                # self.time_coeffs is (Ns, n_modes_save)
                # self.modes is (Nspace, n_modes_save)
                reconstructed_snapshot_k = self.time_coeffs[snap_idx, :k_modes] @ self.modes[:, :k_modes].T
    ...
    def check_spatial_mode_orthogonality(self, tolerance=1e-9):
        """Check the orthogonality of spatial modes with respect to weights W.
           Verifies that Modes.T @ W_diag @ Modes is close to Identity.
        """
    ...
    def check_temporal_coefficient_orthogonality(self, tolerance=1e-9):
        ...
        """
        if self.time_coeffs.size == 0 or self.eigenvalues.size == 0 or 'Ns' not in self.data:
        ...
        Ns = self.data['Ns']
        n_saved_coeffs = self.time_coeffs.shape[1]
        ...
        # self.time_coeffs = Psi_temp * sqrt(eigenvalues_temp * Ns)
        # So (1/Ns) * self.time_coeffs.T @ self.time_coeffs = 
        # (1/Ns) * sqrt(L*Ns) * Psi_temp.T @ Psi_temp * sqrt(L*Ns) = L (diag(eigenvalues))
        ortho_check_matrix = (1.0 / Ns) * (self.time_coeffs.T @ self.time_coeffs)
    ...
    def run_analysis(self, plot_n_modes_spatial=4, plot_n_coeffs_time=2, 
                     plot_snapshot_indices=None, plot_modes_for_reconstruction=None,
                     check_orthogonality=True):
        """
        Run the full POD analysis pipeline: load, compute, save, plot.

        Args:
            plot_n_modes_spatial (int): Number of spatial modes to plot.
            plot_n_coeffs_time (int): Number of temporal coefficients to plot.
            plot_snapshot_indices (list of int, optional): Snapshot indices for reconstruction plot.
            plot_modes_for_reconstruction (list of int, optional): Mode counts for reconstruction plot.
            check_orthogonality (bool): Whether to perform and plot orthogonality checks.
        """
        print("Starting POD analysis pipeline...")
        start_total_time = time.time()

        # Load data (compute_fft=False as POD doesn't use FFT blocks from BaseAnalyzer)
        ...
        self.plot_reconstruction_comparison(
            snapshot_indices_to_plot=plot_snapshot_indices, 
            modes_for_reconstruction=plot_modes_for_reconstruction
        )
...
if __name__ == "__main__":
    # --- Configuration ---
    # Try with a small dataset first if available, e.g., a subset of jetLES or cavity
    data_file = "./data/jetLES_small.mat" # Example, replace with your data
    # data_file = "./data/cavityPIV_small.mat" # Example for cavity data

    results_dir_main = "./pod_results"   # Directory for HDF5 results
    figures_dir_main = "./pod_figs"      # Directory for plots

    n_modes_to_save_main = 10        # Number of POD modes to save
    n_modes_to_plot_spatial_main = 4 # Number of spatial modes to visualize
    n_coeffs_to_plot_time_main = 2   # Number of temporal coefficients to visualize
    # ---------------------
    ...
    print(f"Results saved in: {os.path.abspath(results_dir_main)}")
    print(f"Figures saved in: {os.path.abspath(figures_dir_main)}")
    print("--------------------------------------------")
